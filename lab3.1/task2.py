...
if number in ["1", "2"]:
        ...
        if func[0] == 1:
            ...
            if calendar.date !=  calendar.subtracting(days=change_day, months=change_month, years=change_year):
                if  calendar.date >  calendar.subtracting(days=change_day, months=change_month, years=change_year):
                        print(f'\t\tthe start date is bigger than the end date')
                if  calendar.date <  calendar.subtracting(days=change_day, months=change_month, years=change_year):
                        print(f'\t\tthe start date is less than the end date')
                elif calendar.date ==  calendar.subtracting(days=change_day, months=change_month, years=change_year):
                    print(f'\t\tThe start date is the same as the end date')
